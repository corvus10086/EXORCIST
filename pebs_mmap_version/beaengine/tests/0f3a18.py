...
class TestSuite:
    ...
    def test(self):
        ...
        Buffer = '{}181033'.format(myVEX.c4()).decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.infos.Instruction.Opcode, 0x18)
        assert_equal(myDisasm.infos.Instruction.Mnemonic, 'vinsertf128 ')
        assert_equal(myDisasm.infos.repr, 'vinsertf128 ymm10, ymm0, xmmword ptr [r8], 33h')

        # EVEX.256.66.0F3A.W0 18 /r ib
        # VINSERTF32X4 ymm1 {k1}{z}, ymm2, xmm3/m128, imm8

        myEVEX = EVEX('EVEX.256.66.0F3A.W0')
        Buffer = '{}182011'.format(myEVEX.prefix()).decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.infos.Instruction.Opcode, 0x18)
