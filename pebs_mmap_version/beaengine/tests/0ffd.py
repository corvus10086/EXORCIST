...
class TestSuite:
    def test(self):
        ...
        assert_equal(hex(myDisasm.infos.Instruction.Opcode), '0xffd')
        assert_equal(myDisasm.infos.Instruction.Mnemonic, 'paddw ')
        assert_equal(myDisasm.infos.repr, 'paddw xmm2, xmmword ptr [rax+44332211h]')

        # VEX.NDS.128.66.0F.WIG fd /r
        # vpaddw xmm1, xmm2, xmm3/m128
        Buffer = 'c40101fd0e'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.infos.Instruction.Mnemonic, 'vpaddw ')
        assert_equal(myDisasm.infos.repr, 'vpaddw xmm9, xmm15, xmmword ptr [r14]')

        # VEX.NDS.256.66.0F.WIG fd /r
        # vpaddw ymm1, ymm2, ymm3/m256
        Buffer = 'c40105fd0e'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.infos.Instruction.Mnemonic, 'vpaddw ')
        assert_equal(myDisasm.infos.repr, 'vpaddw ymm9, ymm15, ymmword ptr [r14]')

        # EVEX.NDS.128.66.0F.WIG fd /r
        # vpaddw xmm1 {k1}{z}, xmm2, xmm3/m128
        Buffer = '62010506fd0e'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        ...
        assert_equal(myDisasm.infos.Instruction.Mnemonic, 'vpaddw ')
        assert_equal(myDisasm.infos.repr, 'vpaddw xmm25, xmm31, xmmword ptr [r14]')

        # EVEX.NDS.256.66.0F.WIG fd /r
        # vpaddw ymm1 {k1}{z}, ymm2, ymm3/m256
        Buffer = '62010520fd0e'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.infos.Reserved_.EVEX.P0, 0x1)
        assert_equal(myDisasm.infos.Reserved_.EVEX.P1, 0x5)
        ...
        assert_equal(hex(myDisasm.infos.Instruction.Opcode), '0xfd')
        assert_equal(myDisasm.infos.Instruction.Mnemonic, 'vpaddw ')
        assert_equal(myDisasm.infos.repr, 'vpaddw ymm25, ymm31, ymmword ptr [r14]')

        # EVEX.NDS.512.66.0F.WIG fd /r
        # vpaddw zmm1 {k1}{z}, zmm2, zmm3/m512
        Buffer = '62010540fd0e'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.infos.Reserved_.EVEX.P0, 0x1)
        assert_equal(myDisasm.infos.Reserved_.EVEX.P1, 0x5)
        ...
        assert_equal(hex(myDisasm.infos.Instruction.Opcode), '0xfd')
        assert_equal(myDisasm.infos.Instruction.Mnemonic, 'vpaddw ')
        assert_equal(myDisasm.infos.repr, 'vpaddw zmm25, zmm31, zmmword ptr [r14]')
