...
.L2$pb:
	pop	eax
.Ltmp1:
	add	eax, offset _GLOBAL_OFFSET_TABLE_+(.Ltmp1-.L2$pb)
...
# %bb.13:                               #   in Loop: Header=BB2_11 Depth=2
	mov	eax, dword ptr [ebp - 88]       # 4-byte Reload
	mov	dword ptr [eax + x_is_safe_static@GOTOFF], 1
.LBB2_14:                               #   in Loop: Header=BB2_11 Depth=2
	mov	eax, dword ptr [ebp - 88]       # 4-byte Reload
	lea	eax, [eax + x_is_safe_static@GOTOFF]
...
.LBB2_18:                               #   in Loop: Header=BB2_11 Depth=2
	mov	ebx, dword ptr [ebp - 88]       # 4-byte Reload
	mov	eax, dword ptr [ebp - 32]
	mov	ecx, 6
	cdq
	idiv	ecx
	...
	mov	eax, dword ptr [ebp - 88]       # 4-byte Reload
	mov	dword ptr [eax + x_is_safe_static@GOTOFF], 0
...
.L3$pb:
	pop	eax
.Ltmp2:
	add	eax, offset _GLOBAL_OFFSET_TABLE_+(.Ltmp2-.L3$pb)
